...
export default () => {
    ...
    return (
        <div>
            ...
            <Row> {
                workout && workout.exercises.map((item, idx) => (
                    <Col className="bk-white rounded-corners p-1 mt-1 mb-1"
                        span={22}
                        offset={1}
                        key={idx}
                    >
            ...
            <Row justify="center"><Col span={24} className="text-center"><Button type="primary" className="inv" onClick={onSumbitSession}>End Session</Button></Col></Row>
        </div>
    );
};
...
const WorkoutExercise = ({ item, onUpValues }) => {
    ...
    const onClickWeight = (isUp) => {
        handleOnClick(isUp, setWeight, weight + 1, weight - 1 > 0 ? weight - 1 : weight);
    };
            ...
            <Row justify="space-between" align="center">
                ...
                <Col>
                    <Space size={15}
                        ...
                        <WorkoutSetInputModal entry={item} setModalVisible={setModalVisible} visible={modalVisible} >
                            {
                                exerciseType === "Anaerobic" && (

                                    <AnaerobicForm onAnaerobicSubmit={onAnaerobicSubmit}
                                        values={
                                            [
                                                {
                                                    label: "reps",
                            ...
                            }
                            {
                                exerciseType === "Aerobic" && (
                                    ...
                                    <AerobicForm onAerobicSubmit={onAerobicSubmit}
                                        values={
                                            [{
                                                label: "duration",
                                                value: duration
                    ...
                    <>
                        ...
                        <Row justify="center">
                            <Col xs={20} sm={16} md={12} lg={8}>
                                <div> {values.map((e, index) => {

                                    if (Array.isArray(e)) { // Anaerobic
                                        let [reps, weight] = e;

                                        return (
                                            <>
                                                {(index !== 0) && <Divider key={reps + index} style={{ width: "80%" }} />}
                                                <div key={reps + index} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                    <Title level={5}><b>Set {index + 1}</b></Title>
                                                    <div>
                                                        <p>
                                                            <b>{
                                                                reps.label
                                        ...
                                        return (
                                            <>
                                                {(index !== 0) && <Divider key={e.label + index} style={{ width: "80%" }} />}
                                                <div key={e.label + index} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                    <Title level={5}><b>Set {index + 1}</b></Title>
                                                    <div>
                                                        <p>
                                                            <b>{
...
const WorkoutSetInputModal = ({ entry, children, visible, setModalVisible, handleOk }) => {
    return (
        <>
...
const AnaerobicForm = ({
    ...
    return (
        <>
            <Title level={4}
                className="inv-font text-center">
                New entry
            </Title>
            <Form title="Weight sets"
                ...
                <Row justify="center">
                    <Col span={12}>
                        <InputCounter label={reps.label}
                            value={reps.value}
                            onClick={onClickReps} />
                    </Col>
                    <Col span={12}>
                        <InputCounter label={weight.label}
                            value={weight.value}
                ...
                <Row justify="center" align="middle" >
                    ...
                    <Col span={10} className="flex-md-bl">
                        <Form.Item
                            ...
                            className="flex-md-md"
                        >
                            <InputNumber className="transparent" min={0} max={59} defaultValue={1} />
                        </Form.Item>
